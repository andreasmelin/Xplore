...
CREATE TABLE IF NOT EXISTS activity_log (
  ...
  activity_id TEXT, -- e.g., 'letter_a', 'topic_dinosaurs', 'math_comparing_1'
  activity_name TEXT, -- Human readable name
  duration_seconds INTEGER DEFAULT 0,
  completed BOOLEAN DEFAULT FALSE,
  score INTEGER, -- For activities with scoring (0-100)
...
-- Daily summary statistics (for faster dashboard queries)
CREATE TABLE IF NOT EXISTS daily_stats (
  ...
  letters_practiced TEXT[] DEFAULT '{}', -- Array of letter IDs
  topics_explored TEXT[] DEFAULT '{}', -- Array of topic IDs
  math_activities INTEGER DEFAULT 0,
  chat_messages INTEGER DEFAULT 0,
  average_score INTEGER, -- Average score across scored activities
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
-- Trigger to update daily_stats when activity is logged
...
-- Function to get profile summary for last N days
...
-- Comments
COMMENT ON TABLE activity_log IS 'Detailed log of all learning activities';
COMMENT ON TABLE daily_stats IS 'Aggregated daily statistics for faster dashboard queries';
COMMENT ON FUNCTION get_profile_summary IS 'Get summary stats for a profile over N days';
